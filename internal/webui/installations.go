...
<section class="panel"><div class="panel-head"><div><h2>Immutable installation inventory</h2><p>Studio remains read-only. Authenticated provisioning controls live in the dedicated Operator Console.</p></div><span class="count">{{len .Rows}} total</span></div>
...
{{else}}<div class="empty"><div class="radar"><i></i></div><h3>No InstallationSpecs yet</h3><p>Create the first verified Debian 13 Standard spec through the authenticated Operator Console, then review it here before arming.</p></div>{{end}}</section></main></div></body></html>`
