...
func TestAssignmentRejectsUntilManagedAgentBuildReady(t *testing.T) {
	...
	if _, err := state.SetMachinePolicy(ctx, machineRecord.ID, machine.PolicyProvision, "req_approve_agent_gate", "test:operator"); err != nil {
		t.Fatal(err)
	}
	spec := createAssignmentSpecWithoutReadyAgent(t, state, machineRecord.ID, "req_spec_agent_gate")
	if _, err := state.ArmInstallation(ctx, machineRecord.ID, spec.ID, "req_arm_before_agent", "test:operator"); fault.Code(err) != fault.AgentBuildNotReady {
		t.Fatalf("arm before agent build code=%q err=%v", fault.Code(err), err)
	}
	makeInitialAgentBuildReady(t, state)
	if _, err := state.ArmInstallation(ctx, machineRecord.ID, spec.ID, "req_arm_after_agent", "test:operator"); err != nil {
		t.Fatalf("arm after agent build ready: %v", err)
	}
}
...
func makeInitialAgentBuildReady(t *testing.T, state *Store) {
	...
	if _, err := state.CompleteAgentBuild(context.Background(), build.ID, "aegispxe-agent_test_amd64.deb", strings.Repeat("a", 64), 1024, strings.Repeat("b", 64), testManifestSignature, "req_test_agent_build_ready"); err != nil {
		t.Fatal(err)
	}
}
