...
func TestInstallationAutomaticallyCreatesManagedAgentAndQueuesBuild(t *testing.T) {
	...
	if record.State != agent.StatePendingBuild || record.UpdateMode != agent.UpdateModeManual || record.DesiredGeneration != 1 || record.ActiveGeneration != 0 {
		t.Fatalf("unexpected initial managed agent state: %+v", record)
	}
	if len(record.CapabilityCeiling) != 0 {
		t.Fatalf("unexpected initial capability ceiling: %v", record.CapabilityCeiling)
	}
	build, err := state.AgentBuild(ctx, record.ID, 1)
	if err != nil {
		t.Fatal(err)
	}
	if build.State != agent.BuildStateQueued || build.Generation != 1 || build.Architecture != spec.Architecture {
		t.Fatalf("unexpected initial build: %+v", build)
	}
	events, err := state.Events(ctx, event.EntityAgent, record.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(events) != 2 || events[0].Type != event.AgentCreated || events[1].Type != event.AgentBuildQueued || events[0].RequestID != "req_agent_spec" || events[1].RequestID != "req_agent_spec" {
...
func TestQueueAgentBuildRejectsGenericShellCapability(t *testing.T) {
	...
	if _, err := state.FailAgentBuild(ctx, mustAgentBuild(t, state, record.ID, 1).ID, fault.AgentBuildFailed, "test failure", "req_fail_initial"); err != nil {
		t.Fatal(err)
	}
	if _, err := state.QueueAgentBuild(ctx, record.ID, []string{"shell"}, "req_agent_invalid", "test:operator"); fault.Code(err) != fault.AgentInvalid {
...
func TestAgentBuildLifecyclePersistsArtifactMetadataAndAudit(t *testing.T) {
	...
	if claimedRecord.ID != record.ID || build.State != agent.BuildStateBuilding || build.Version != "0.2.0-dev.1" {
		t.Fatalf("unexpected claimed build: record=%+v build=%+v", claimedRecord, build)
	}
	...
	if ready.State != agent.BuildStateReady || ready.PackageSHA256 != shaA || ready.ManifestSHA256 != shaB || ready.ManifestSignature != testManifestSignature {
		t.Fatalf("unexpected ready build: %+v", ready)
	}
	persisted, err := state.AgentBuild(ctx, record.ID, 1)
	if err != nil {
		t.Fatal(err)
	}
	if persisted.State != agent.BuildStateReady || persisted.PackagePath != ready.PackagePath || persisted.ManifestSignature != ready.ManifestSignature {
		t.Fatalf("unexpected persisted build: %+v", persisted)
	}
	updated, err := state.ManagedAgent(ctx, record.ID)
	if err != nil {
		t.Fatal(err)
	}
	if updated.State != agent.StateReady {
		t.Fatalf("agent state=%q want=%q", updated.State, agent.StateReady)
	}
	events, err := state.Events(ctx, event.EntityAgent, record.ID)
	if err != nil {
		t.Fatal(err)
	}
	if len(events) != 4 || events[2].Type != event.AgentBuildStarted || events[3].Type != event.AgentBuildReady {
...
func TestCompleteAgentBuildRejectsMalformedSignature(t *testing.T) {
	...
	if fault.Code(err) != fault.AgentBuildInvalid {
		t.Fatalf("malformed signature code=%q err=%v", fault.Code(err), err)
	}
}
