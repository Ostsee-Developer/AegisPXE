...
`0.1.0-dev.8` adds the authenticated Operator Console and first Debian 13 Standard InstallationSpec wizard. A dedicated operator listener defaults to loopback `127.0.0.1:8091` and rejects non-loopback configuration, while the provisioning listener remains available for PXE/read-only Studio traffic. The Console exposes Machine policy and assignment mutations through the existing session/CSRF/audit boundary. The wizard validates ProfileSnapshot and whole-device target input before network work, resolves Debian artifacts through the signed trust chain, freezes only verified descriptors into the immutable InstallationSpec, and keeps creation separate from the explicit arm action. HTTP write bounds are aligned with the verified artifact transport so a valid two-minute artifact fetch is not cut off by a shorter server response timeout.
