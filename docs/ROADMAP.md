...
`0.1.0-dev.7` introduces the minimal bootstrap operator authentication boundary required before administrative mutations. A local 256-bit bootstrap key is exchanged over direct TLS or loopback for an 8-hour server-side session. Browser mutations require session-bound CSRF, login attempts are rate-limited, and cleartext non-loopback HTTP remains read-only. Machine policy, Installation arm and Assignment cancel endpoints are protected by this wrapper.

`0.1.0-dev.8` adds the authenticated Operator Console and first Debian 13 Standard InstallationSpec wizard. A dedicated operator listener defaults to loopback `127.0.0.1:8091`. The Console exposes Machine policy and assignment mutations through the existing session/CSRF/audit boundary. The wizard validates ProfileSnapshot and whole-device target input before network work, resolves Debian artifacts through the signed trust chain, freezes only verified descriptors into the immutable InstallationSpec, and keeps creation separate from the explicit arm action. HTTP write bounds are aligned with the verified artifact transport.

`0.1.0-dev.9` separates PXE and Studio into distinct listener roles and fixes the package default that accidentally made PXE HTTP loopback-only. The PXE listener is network reachable and exposes only health/discovery/boot transport. Studio has its own path allowlist and remains loopback by default. Non-loopback Studio deployment requires explicit trusted reverse-proxy CIDRs plus configurable HTTPS/identity headers; forged proxy headers from any other direct peer fail closed. Trusted proxy identities receive normal AegisPXE server-side sessions and CSRF while the bootstrap key remains a local recovery path. No external Studio/domain URL is compiled into AegisPXE.

The same dev.9 slice hardens the real Debian finish path without suppressing failures. The Preseed late hook records step markers and final exit status in the target system, prepares `/run/sshd`, ensures host keys and validates sshd/sudoers before Debian's native unattended finish continues. A hardening failure remains a failure rather than being converted to success.

The next hard boundary is cryptographic boot trust plus authenticated installer telemetry. AegisPXE must establish installation-scoped proof with freshness/replay protection before releasing lifecycle credentials or accepting `INSTALLER_STARTED`, later stages, installer logs or validation results. TPM-backed attestation is the preferred first hardware-backed path for capable systems; any non-TPM fallback requires an explicit security decision and must not silently downgrade.
